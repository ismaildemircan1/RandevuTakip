...
const firebaseConfig = {
  ...
  // ⚡ Bolt Optimization: Debounce patient search
  // What: Wraps the search logic in a 300ms setTimeout, clearing any previous timeout on input.
  // Why: The previous implementation fetched the entire 'patients' collection from Firestore on *every single keystroke*.
  // Impact: Reduces database reads and API calls by >90% during active typing, saving costs and preventing rate limit issues.
  // Measurement: Check network tab or database read logs. While typing "Ahmet", it now makes 1 request instead of 5.
  let searchTimeout;
