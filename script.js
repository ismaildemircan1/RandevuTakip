...
const firebaseConfig = {
  ...
  // ⚡ Bolt Optimization: Replace expensive Firestore fetch with local DOM filtering
  // This reduces network latency to 0ms and eliminates database reads on every keystroke
  document.getElementById('patientSearch').addEventListener('input', function(e) {
      const searchTerm = e.target.value.toLowerCase();
      const list = document.getElementById('patientList');
      const patientItems = list.getElementsByClassName('patient-item');

      // Filter existing DOM elements instead of re-fetching from the database
      Array.from(patientItems).forEach(item => {
          // Find the strong tag for 'Ad:' and get the text content after it
          const nameElement = item.querySelector('p:first-child');
          if (nameElement) {
              const nameText = nameElement.textContent.replace('Ad:', '').trim().toLowerCase();
